...
function FundersPage() {
  ...
  const fetchFundingRequests = async () => {
    try {
      const fundingRequests = await contract.listFundingRequests();
      // const formattedRequests = fundingRequests.map((request, index) => ({
      //   title: request[1],
      //   researcher: request[0],
      //   description: request[2],
      //   amount: request[3].toString(),
      //   vote: request[5].toString(),
      //   totalAmountCollected: contract.indAmount(index).toString() // Access the indAmount mapping
      // }));
      const formattedRequests = await Promise.all(
        fundingRequests.map(async (request, index) => {
          const totalAmountCollected = await contract.indAmount(index);
          return {
            title: request[1],
            researcher: request[0],
            description: request[2],
      ...
      setRequests(formattedRequests);
      setButtonClicked(true);
      console.log(contract.indAmount[0])
      const balance = await contract.getContractBalance();
      setContractBalance(balance.toString());
    } catch (error) {
      console.error('Error fetching funding requests:', error);
          ...
          {requests.map((request, index) => {
            ...
            return (
              <Card key={index} title={request.title} className="funding-card">
                ...
                <p><strong>Amount Requested:</strong> {request.amount}</p>
                <p><strong>Votes Received:</strong> {request.vote}</p>
                <p><strong>Total Amount Collected:</strong> {request.totalAmountCollected}</p> {/* Display the total amount collected */}
                <input
                  type="number"
                  placeholder="Enter amount to fund"
                  onChange={(e) => handleAmountChange(index, e.target.value)}
